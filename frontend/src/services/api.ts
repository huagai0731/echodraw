...
export type VisualAnalysisResultRecord = {
  ...
  comprehensive_analysis?: any; // 专业分析结果（JSON格式）
  created_at: string;
  updated_at: string;
};
...
export type CreateVisualAnalysisResultInput = {
  ...
  step5_hue: string;
  binary_threshold: number;
  comprehensive_analysis?: any; // 专业分析结果（可选）
};
...
export async function createVisualAnalysisResult(
  ...
  if (input.comprehensive_analysis) {
    formData.append('comprehensive_analysis', JSON.stringify(input.comprehensive_analysis));
  }
 ...
 * 专业绘画分析（综合分析）- 创建异步任务
 */
export async function analyzeImageComprehensive(imageData: string): Promise<{ task_id: string; status: string; progress: number }> {
  if (!hasAuthToken()) {
    throw createUnauthorizedError();
  }

  const response = await api.post("/visual-analysis/comprehensive/", {
