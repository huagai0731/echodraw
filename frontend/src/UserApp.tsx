...
type PageId = NavId | "upload" | "profile-achievements" | "test-list" | "test-taking" | "test-results" | "mental-state-assessment" | "color-perception-test" | "color-test-results" | "visual-analysis" | "calendar";
...
function UserApp() {
  ...
  const refreshUserArtworks = useCallback(async (forceRefresh = false) => {
    ...
    const executeRefresh = async () => {
      ...
      try {
        ...
        console.log("[Echo] refreshUserArtworks: filtered artwork records count:", artworkRecords.length);
        
        if (artworkRecords.length > 0) {
          console.log("[Echo] First artwork record tags:", artworkRecords[0].tags);
        }
  ...
  const handleVisualAnalysisSave = useCallback(async (result: any) => {
    try {
      ...
      console.error("保存视觉分析结果失败:", error);
      if (typeof window !== "undefined" && typeof window.alert === "function") {
