...
function LongTermGoalSetup({
  ...
  const minCheckpoints = useMemo(() => {
    ...
    // 每个检查点最多999小时，最少5小时
    const minByMax = Math.ceil(totalHours / MAX_HOURS_PER_CHECKPOINT);
    const minByMin = Math.ceil(totalHours / MIN_HOURS_PER_CHECKPOINT);
    // 取两者中的较大值，确保同时满足两个限制
    return Math.max(minByMax, minByMin);
  }, [totalHours]);
...
function clampCheckpointCount(value: number, totalHours: number) {
  ...
  // 每个检查点最多999小时，最少5小时，计算最小检查点数量
  const MAX_HOURS_PER_CHECKPOINT = 999;
  const MIN_HOURS_PER_CHECKPOINT = 5;
  const minByMax = Math.ceil(normalizedTotal / MAX_HOURS_PER_CHECKPOINT);
  const minByMin = Math.ceil(normalizedTotal / MIN_HOURS_PER_CHECKPOINT);
  // 取两者中的较大值，确保同时满足两个限制
  const minCheckpoints = Math.max(minByMax, minByMin);
