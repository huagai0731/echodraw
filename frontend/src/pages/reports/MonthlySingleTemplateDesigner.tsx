...
const GRID_ROWS = 5; // 5周
const TOTAL_CELLS = 35; // 7x5 = 35个格子
...
function MonthlySingleTemplateDesigner({ open, artworks, onClose }: MonthlySingleTemplateDesignerProps) {
  ...
  const placeholderPositions = useMemo(() => {
    ...
    // 例如：如果1号是周六（5），那么周一到周五是0-4，需要5个占位符
    const leadingPlaceholders = firstDayMondayIndex;
    
    // 计算结尾占位符数量（从最后一天到周日之间的天数）
    // 例如：如果30号是周日（6），那么不需要结尾占位符
    // 如果30号是周六（5），那么需要1个占位符（周日）
    ...
    if (trailingPlaceholders > 0) {
      ...
      // 计算总格子数（不超过35）
      const totalCells = Math.min(totalRows * GRID_COLS, TOTAL_CELLS);
      ...
      const actualTrailingPlaceholders = Math.min(trailingPlaceholders, TOTAL_CELLS - actualTrailingStart);
  ...
  }, [gridImages, open, templateData, placeholderPositions, monthDate, imageSizePreset]);
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        if (document.fonts && document.fonts.ready) {
          document.fonts.ready.then(() => {
            setTimeout(() => {
              drawTemplate(exportContext, CANVAS_WIDTH, canvasHeight, templateData, reloadedGridImages, placeholderPositions, monthDate, imageSizePreset, artworks);
              resolve();
            }, 50);
          });
        } else {
              ...
              <section>
                {(() => {
                  ...
                  if (imageSizePreset === "square") {
                    ...
                    const maxCellHeight = (canvasHeight - totalGapHeight) / GRID_ROWS;
...
function drawTemplate(
  ...
  if (imageSizePreset === "square") {
    ...
    const maxCellHeight = (availableHeight - totalGapHeight) / GRID_ROWS;
    // 使用较小的值，确保所有cell都能显示
    const targetSize = Math.min(maxCellWidth, maxCellHeight);
    cellWidth = targetSize;
    cellHeight = targetSize;
...
function drawImageCell(
  ...
  if (!image || image.width === 0 || image.height === 0) {
    // 没有图片时，绘制和占位符一样的样式，包括EchoDraw字样
