...
function calculateMonthlyStats(
  ...
  return {
    ...
    newAchievements: 0, // TODO: 需要从成就API获取
  };
}
...
function FullMonthlyReport({ open, onClose, targetMonth }: FullMonthlyReportProps) {
  ...
  const matchTemplate = useCallback((
    ...
    for (const template of activeTemplates) {
      // 检查条件
      if (template.min_total_uploads !== null && stats.totalUploads < template.min_total_uploads) {
        continue;
      }
      if (template.max_total_uploads !== null && stats.totalUploads > template.max_total_uploads) {
        continue;
      }
      if (template.min_total_hours !== null && stats.totalHours < template.min_total_hours) {
        continue;
      }
      if (template.max_total_hours !== null && stats.totalHours > template.max_total_hours) {
        continue;
      }
      if (template.min_avg_hours !== null && stats.avgHoursPerUpload < template.min_avg_hours) {
        continue;
      }
      if (template.max_avg_hours !== null && stats.avgHoursPerUpload > template.max_avg_hours) {
        continue;
      }
      if (template.min_avg_rating !== null && stats.avgRating < template.min_avg_rating) {
        continue;
      }
      if (template.max_avg_rating !== null && stats.avgRating > template.max_avg_rating) {
        continue;
      }
      return template;
    }
  ...
  const renderTemplateText = useCallback((
    ...
    // 替换变量并添加高亮
    let text = template
      .replace(/{count}/g, String(stats.totalUploads))
      .replace(/{hours}/g, stats.totalHours.toFixed(1))
      .replace(/{avg_hours}/g, stats.avgHoursPerUpload.toFixed(1))
      .replace(/{rating}/g, stats.avgRating.toFixed(1));
  ...
  return (
    <div className="full-monthly-report" role="dialog" aria-modal="true" aria-label="完整月报">
      ...
      <div className="full-monthly-report__container">
                    ...
                    <div className="full-monthly-report__screen-content">
                      <div className="rhythm__container">
                        ...
                        <div className="rhythm__summary">
                          <p>
                            你在<span className="rhythm__summary-highlight">周末</span>更容易产出长稿，尝试将同样的节奏延续到<span className="rhythm__summary-highlight">周三晚</span>。
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="full-monthly-report__screen-content">
                      <div className="milestone__container">
                        <div className="milestone__summary">
                          <p>
                            {lastUpload && lastUpload.self_rating && firstUpload && firstUpload.self_rating && 
                             (lastUpload.self_rating || 0) >= (firstUpload.self_rating || 0) ? (
                              <>你的最后一张作品是评分最高的，整体节奏逐步走向稳定。</>
                            ) : (
                              <>你的创作节奏正在调整中，继续探索适合自己的节奏。</>
                            )}
                          </p>
                        </div>
                    ...
                    <div className="full-monthly-report__screen-content">
                      <div className="depth__container">
                        ...
                        <div className="depth__insight">
                          <p>
                            你是典型的<span className="depth__insight-highlight">碎片型创作者</span>，擅长在短时间内捕捉灵感。尝试安排<span className="depth__insight-highlight">整块时间</span>，或许能开启更深度的创作旅程。
                          </p>
                        </div>
                      </div>
                    </div>
