...
import { DurationTagSettings, type DurationTagSettingsState } from "@/components/templateDesigner/DurationTagSettings";
...
type WeeklySingleTemplateDesignerProps = {
  open: boolean;
  artworks: Artwork[];
  onClose: () => void;
};
...
function WeeklySingleTemplateDesigner({ open, artworks, onClose }: WeeklySingleTemplateDesignerProps) {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [_username, _setUsername] = useState<string>(DEFAULT_USERNAME);
  const [_title, _setTitle] = useState<string>("自定义标题名");
  const [previewImageUrl, setPreviewImageUrl] = useState<string | null>(null);
  const [showPreviewModal, setShowPreviewModal] = useState(false);
  const [_subtitle, _setSubtitle] = useState<string>("自定义文案");
  ...
  const [_showTitle, _setShowTitle] = useState(true);
  const [_showSubtitle, _setShowSubtitle] = useState(true);
  const [_showUsername, _setShowUsername] = useState(true);
  const [_showDate, _setShowDate] = useState(true);
  const [_showDuration, _setShowDuration] = useState(true);
  // 阴影区透明度设置状态
  const [shadowOpacitySettings, setShadowOpacitySettings] = useState<ShadowOpacitySettingsState>({
    opacity: 85,
  });
  // 时长标签设置状态
  const [durationTagSettings, setDurationTagSettings] = useState<DurationTagSettingsState>({
    show: true,
    opacity: 80,
  });
  ...
  const [imageSizePreset, setImageSizePreset] = useState<ImageSizePreset>("square");
  // 内容编辑状态
  ...
  // 九宫格图片选择：7个可编辑位置（排除位置2和6，它们是占位）
  // 位置索引：0,1,2 | 3,4,5 | 6,7,8
  // 位置2和6是占位，其他7个可编辑
  const [gridImages, setGridImages] = useState<GridImage[]>(
    new Array(9).fill(null).map(() => ({ artworkId: null, image: null })),
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          console.warn("[WeeklySingleTemplateDesigner] 无法加载用户昵称：", error);
  ...
  useEffect(() => {
    ...
    loadTagPreferencesAsync()
      ...
      .catch((error) => {
        console.warn("[WeeklySingleTemplateDesigner] Failed to load tag options:", error);
      });
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    ...
    // getOrLoadImage已经设置了crossOrigin，直接使用
  ...
  const weekInfo = useMemo(() => {
    ...
    // 获取月初第一天是星期几（0=周日，1=周一，...）
    const firstDayWeekday = firstDayOfMonth.getDay();
    // 转换为周一为0的格式
    const firstDayMondayIndex = firstDayWeekday === 0 ? 6 : firstDayWeekday - 1;
    
    // 计算start是月初第几天
    const daysFromMonthStart = start.getDate() - 1;
    // 计算start是第几周（从0开始，所以+1）
    const weekNumber = Math.floor((firstDayMondayIndex + daysFromMonthStart) / 7) + 1;
  ...
  const templateData = useMemo<TemplateViewModel | null>(() => {
    ...
    return {
      durationLabel,
      overlayOpacity,
      durationTagShow: durationTagSettings.show,
      durationTagOpacity: clamp01(durationTagSettings.opacity / 100),
  ...
  const getFilteredArtworks = (position: number): Artwork[] => {
    const positionToDayOffset: Record<number, number> = {
      0: 0, // 周一
      1: 1, // 周二
      3: 2, // 周三
      4: 3, // 周四
  ...
  useEffect(() => {
    ...
    const baseCanvasHeight = imageSizePreset === "square" ? CANVAS_HEIGHT_SQUARE : CANVAS_HEIGHT_RECTANGLE;
    ...
    if (document.fonts && document.fonts.ready) {
      ...
      // 如果浏览器不支持document.fonts，等待一段时间后绘制
      setTimeout(draw, 150);
    }
  }, [gridImages, open, templateData, imageSizePreset]);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const canvasHeight = imageSizePreset === "square" ? CANVAS_HEIGHT_SQUARE : CANVAS_HEIGHT_RECTANGLE;
              ...
              {hasArtworks ? (
                <div className="single-template-designer__device">
                  <div
                    ...
                    style={{ aspectRatio: CANVAS_WIDTH / (imageSizePreset === "square" ? CANVAS_HEIGHT_SQUARE : CANVAS_HEIGHT_RECTANGLE) }}
                  >
            ...
            {hasArtworks ? (
              <section>
                <div className="single-template-designer__group">
                  ...
                  <div className="single-template-designer__field-row single-template-designer__field-row--inline" style={{ gap: "0.5rem" }}>
                    <button
                      type="button"
                      className={`single-template-designer__tag-option ${imageSizePreset === "square" ? "single-template-designer__tag-option--active" : ""}`}
                    ...
                    <button
                      type="button"
                      className={`single-template-designer__tag-option ${imageSizePreset === "rectangle" ? "single-template-designer__tag-option--active" : ""}`}
                      onClick={() => setImageSizePreset("rectangle")}
                    >
                      1080×1350
                    </button>
                  </div>
                ...
                <DurationTagSettings
                  state={durationTagSettings}
                  onChange={setDurationTagSettings}
                />
              </section>
            ) : null}
            ...
            {hasArtworks ? (
              <section>
                <ImageGridSelector
                  ...
                  artworks={artworks}
                  placeholderPositions={[2, 6]}
                  placeholderLabels={{ 2: "周信息", 6: "EchoDraw" }}
                  ...
                  targetHeight={imageSizePreset === "square" ? CANVAS_HEIGHT_SQUARE : CANVAS_HEIGHT_RECTANGLE}
                />
                ...
                <div className="single-template-designer__actions">
                  ...
                  <p>导出 PNG · {CANVAS_WIDTH} × {imageSizePreset === "square" ? CANVAS_HEIGHT_SQUARE : CANVAS_HEIGHT_RECTANGLE} 像素 · 适配社交媒体展示。</p>
                </div>
              </section>
            ) : null}
          </div>
...
export default WeeklySingleTemplateDesigner;
...
async function resolveActiveUsername(): Promise<string> {
  ...
  try {
    ...
    console.warn("[WeeklySingleTemplateDesigner] 无法获取用户昵称配置：", error);
    return fallback;
  }
}
...
// 注意：Canvas 2D API不直接支持font-feature-settings，使用SVG+foreignObject来渲染
// 返回Promise以便等待异步渲染完成
...
function drawTemplate(
  ...
  // 网格索引映射：0,1,2 | 3,4,5 | 6,7,8
  // 位置3和7是占位（右上角和左下角）
  // 其他7个位置可编辑
  const gridPositions = [
    { row: 0, col: 0, index: 0 }, // 左上
    { row: 0, col: 1, index: 1 }, // 上中
    { row: 0, col: 2, index: 2 }, // 右上（周信息）
    { row: 1, col: 0, index: 3 }, // 左中
    { row: 1, col: 1, index: 4 }, // 中心
    { row: 1, col: 2, index: 5 }, // 右中
    { row: 2, col: 0, index: 6 }, // 左下（EchoDraw）
    { row: 2, col: 1, index: 7 }, // 下中
    { row: 2, col: 2, index: 8 }, // 右下
  ];
  ...
  gridPositions.forEach(({ row, col, index }) => {
    ...
    if (index === 2) {
      // 位置2：右上角周信息
      drawWeekInfo(context, x, y, cellWidth, cellHeight, data);
    } else if (index === 6) {
      // 位置6：左下角EchoDraw
...
function drawImageCell(
  ...
  if (image && image.width > 0 && image.height > 0) {
    // 直接绘制图片
    // 如果图片是通过blob URL加载的（从fetch获取），应该不会污染Canvas
  ...
  context.restore(); // 恢复 clip 状态，这样后面的文字就不会被裁剪

  // 如果没有图片，绘制占位符边框和对角线（和左下角EchoDraw完全一样）
  if (!image || image.width === 0 || image.height === 0) {
    // 绘制圆角矩形边框，边框颜色是薄荷绿色
    ...
    // 绘制完整的虚线对角线，从框的四个角画到对角（和EchoDraw完全一样）
    const lineWidth = 1;
    context.strokeStyle = "rgba(186, 211, 204, 0.6)"; // 降低饱和度的颜色
    context.lineWidth = lineWidth;
    
    // 设置虚线样式
    const dashPattern = [4, 4]; // 4像素实线，4像素空白
...
function drawWeekInfo(
  ...
  const paddingY = height * 0.25; // 增加paddingY，让整体内容下移
  ...
  const monthFontSize = Math.round(width * 0.4); // 月份数字非常大，占据两行
  const slashFontSize = Math.round(width * 0.35); // 斜杠也很大
  const weekFontSize = Math.round(width * 0.12); // WEEK5较小，放在右下角
  const dateFontSize = Math.round(width * 0.09); // 日期更小
  ...
  const weekX = contentX + numberWidth + slashWidth + Math.round(width * 0.02);
  ...
  const dateY = baseLineY + Math.round(monthFontSize * 0.3); // 在11/下方
  context.font = `400 ${dateFontSize}px "Ethereal", "Manrope", "Segoe UI", sans-serif`;
  context.fillStyle = "#98dbc6";
  context.fillText(data.weekInfo.dateRange, contentX, dateY);

  // 在格子右下角添加用户署名（放大一点）
  if (data.username) {
    const usernameFontSize = Math.round(width * 0.05); // 从0.035放大到0.05
    const usernamePadding = Math.round(width * 0.02);
    const usernameX = x + width - usernamePadding;
    const usernameY = y + height - usernamePadding;
...
function drawEchoDraw(
  ...
  // 绘制圆角矩形边框，边框颜色是薄荷绿色
  context.strokeStyle = "#98dbc6"; // 薄荷绿色边框
  context.lineWidth = 1;
  drawRoundedRectPath(context, cellX, cellY, cellWidth, cellHeight, radius);
  context.stroke();
  ...
  // 绘制完整的虚线对角线，从框的四个角画到对角
  const lineWidth = 1;
  // 降低饱和度：将#98dbc6转换为HSL，降低饱和度
  // #98dbc6 = rgb(152, 219, 198) ≈ hsl(162, 45%, 73%)
  // 降低饱和度到约20%: hsl(162, 20%, 73%) ≈ rgb(186, 211, 204)
  context.strokeStyle = "rgba(186, 211, 204, 0.6)"; // 降低饱和度的颜色
  context.lineWidth = lineWidth;
  ...
  // EchoDraw - 使用Ethereal字体，启用连笔和文体替代字，字体很小，透明度很低
  context.textAlign = "center";
  context.textBaseline = "middle";
  const fontSize = Math.round(cellWidth * 0.08); // 进一步减小字体
  const textColor = "rgba(152, 219, 198, 0.1)"; // 更低的透明度
