...
class VisualAnalysisResultSerializer(serializers.ModelSerializer):
    ...
    def to_internal_value(self, data):
        ...
        for field_name in image_fields:
            # step4_hsv_s 是可选的，允许为空
            if field_name == 'step4_hsv_s' and (field_name not in data or not data[field_name]):
                continue
            ...
            if field_name in data and data[field_name]:
                ...
                if hasattr(image_data, 'read') or hasattr(image_data, 'file'):
                    # 已经是文件对象，不需要转换
                    continue
                ...
                if isinstance(image_data, str) and image_data.strip():
                    ...
                    try:
                        ...
                        raise serializers.ValidationError({
                            field_name: [f"无法解析base64图片: {str(e)}"]
                        })
        ...
        for field_name in image_fields:
            if field_name in data and data[field_name]:
                ...
                if field_value and hasattr(field_value, 'url'):
                    ...
                    if request:
                        data[field_name] = request.build_absolute_uri(field_value.url)
                    else:
