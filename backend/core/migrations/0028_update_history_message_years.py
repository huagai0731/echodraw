...
def update_history_message_years(apps, schema_editor):
    ...
    # 先删除所有旧数据（2000年的）
    history_model.objects.filter(date__year=2000).delete()
...
def reverse_update_history_message_years(apps, schema_editor):
    del schema_editor  # Unused.
    # 不需要反向操作，保留数据
    pass
...
class Migration(migrations.Migration):
    dependencies = [
        ("core", "0027_seed_daily_history_messages"),
    ]

    operations = [
        migrations.RunPython(
            update_history_message_years,
            reverse_update_history_message_years,
        ),
    ]
