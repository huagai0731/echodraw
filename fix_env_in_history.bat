@echo off
REM Windows 批处理脚本：从 Git 历史中移除 backend/.env

echo 检查 Git 历史中是否包含 backend/.env...
git log --all --full-history --oneline -- backend/.env >nul 2>&1
if %errorlevel% equ 0 (
...
echo.
echo 注意：强制推送会重写远程历史，其他团队成员需要重新克隆仓库
pause
